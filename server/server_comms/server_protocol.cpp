...
void ServerProtocol::send_message(const Gamestate& message)
{
    ...
    send_single_int(message.is_running);
    send_single_int(message.is_jumping);
    send_single_int(message.is_flapping);
    send_single_int(message.move_direction);
